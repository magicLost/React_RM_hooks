...
class CalcTranslateX {
    ...
    onPointerDown = (pageX, pageY, listRef, containerRef) => {

        this.pageXStart = pageX;
        this.pageYStart = pageY;
        this.prevPageX = pageX;

        const offsetX = Math.abs(containerRef.current.getBoundingClientRect().x)
        this.translateX = listRef.current.getBoundingClientRect().x - offsetX;
